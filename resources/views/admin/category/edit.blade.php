...
@section('content')
        ...
        <div class="page-header">
          <div class="container-fluid d-flex align-items-center">
            ...
            <h2 class="h5 no-margin-bottom">{{$category->name}} (edit)</h2>
          </div>
        </div>
        ...
        <form action="{{route('admin.dashboard.categories.update',$category->id)}}" method="post">
            ...
            <div class="mx-auto" style="max-width:400px">
                @if (session('successResponse'))
                 ...
                 @endif
                <input type="text" name="name" value="{{$category->name}}" class="form-control mb-3" placeholder="Set the new name of {{$category->name}} category">
                @if ($errors->any())
